...
const InvoicePage = () => {
    ...
    const fetchInvoices = async () => {
        ...
        try {
            const response = await fetch('http://localhost:5000/api/invoices/my-invoices', { headers: { 'Authorization': `Bearer ${token}` } });
    ...
    const fetchCatalogItems = async () => {
        try {
            const token = localStorage.getItem('accessToken');
            const response = await fetch('http://localhost:5000/api/items', { headers: { 'Authorization': `Bearer ${token}` } });
            const data = await response.json();
            if(response.ok) {
                const formattedItems = data.map(item => ({ label: item.nama_item, price: item.harga, isCustom: false }));
                setPredefinedItems([{ label: "Pilih Item...", price: 0, isCustom: false }, ...formattedItems, { label: "Lainnya (Custom)", price: 0, isCustom: true }]);
            }
        } catch (error) {}
    };

    const handleAddItem = () => setItems([...items, { tipe: '', deskripsi: '', qty: 1, harga: 0, isCustom: false }]);
    ...
    const handleItemTextChange = (index, field, value) => { const newItems = [...items]; newItems[index][field] = value; setItems(newItems); };
    const handleRemoveItem = (index) => setItems(items.filter((_, i) => i !== index));

    const handleSubmit = async (e) => {
        e.preventDefault();
        const isInvalidItem = items.some(item => !item.deskripsi || item.harga <= 0 || item.qty <= 0);
        if (isInvalidItem) return Swal.fire({ icon: 'warning', title: 'Data Belum Lengkap', text: 'Pastikan item telah dipilih.'});

        const result = await Swal.fire({ title: 'Buat Invoice?', text: "Pastikan data sudah benar.", icon: 'question', showCancelButton: true });
        ...
        <div className="invoice-page-bg">
            ...
            <div className="main-content-container">
                <div className="page-header">
                    ...
                    <button className="btn-action-primary" onClick={() => setShowModal(true)}>+ Buat Invoice Baru</button>
                </div>

                <div className="invoice-table-container">
                    <table className="invoice-table">
                        <thead><tr><th>No. Invoice</th><th>Tanggal</th><th>Total Tagihan</th><th>Status</th><th>Aksi</th></tr></thead>
                        <tbody>
                            {invoices.length > 0 ? (
                                invoices.map((inv) => (
                                    <tr key={inv.id_invoice}>
                                        <td className="fw-bold">{inv.nomor_invoice}</td><td>{inv.tanggal_invoice}</td><td className="text-orange fw-bold">{formatRupiah(inv.total)}</td>
                                        <td><span className={`status-badge ${inv.status?.toLowerCase().replace(' ', '-')}`}>{inv.status}</span></td>
                                        <td><button className="btn-small-outline" onClick={() => navigate(`/invoice/${inv.id_invoice}`)}>Detail</button></td>
                                    </tr>
                                ))
                            ) : (<tr><td colSpan="5" className="empty-data-row">Belum ada invoice.</td></tr>)}
                        </tbody>
                    </table>
                </div>
            </div>
